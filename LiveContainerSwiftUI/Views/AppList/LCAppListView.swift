...
struct LCAppListView : View, LCAppBannerDelegate, LCAppModelDelegate {
    ...
    func installIpaFile(_ url:URL) async throws {
        try await installIpaFileUsingService(url)
    }

    private func installIpaFileUsingService(_ url: URL) async throws {
        let installed = try await LCAppInstallationService.shared.installFromIPAForUI(at: url) { candidates, preferredName in
            if candidates.contains(where: { $0.uiIsHidden }) && !sharedModel.isHiddenAppUnlocked {
                guard try await LCUtils.authenticateUser() else { return nil }
            }
            var newName = preferredName
            if candidates.isEmpty && !FileManager.default.fileExists(atPath: LCPath.bundlePath.appendingPathComponent(newName).path) {
                return .new(relativeBundlePath: newName)
            }
            let baseName = (preferredName as NSString).deletingPathExtension
            newName = "\(baseName)_\(Int(CFAbsoluteTimeGetCurrent())).app"
            self.installOptions = [AppReplaceOption(isReplace: false, nameOfFolderToInstall: newName)]
            self.installOptions.append(contentsOf: candidates.map {
                AppReplaceOption(isReplace: true, nameOfFolderToInstall: $0.appInfo.relativeBundlePath, appToReplace: $0)
            })
            guard let selected = await self.installReplaceAlert.open() else { return nil }
            if selected.isReplace, let model = selected.appToReplace { return .replace(model) }
