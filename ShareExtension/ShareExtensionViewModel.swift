...
final class ShareExtensionViewModel: ObservableObject {
    ...
    init() {
        ...
        appGroupRootURL = LCSharedUtils.appGroupPath()?.appendingPathComponent("XToolRunner")
    ...
    func installSharedFileInLiveContainer(context: NSExtensionContext?) async {
        ...
        do {
            ...
            guard var components = URLComponents(string: "xtoolrunner://install") else {
    ...
    private func launchBuiltInSideStore(context: NSExtensionContext?) throws {
        ...
        sharedDefaults?.set("xtoolrunner", forKey: "LCLaunchExtensionScheme")
        ...
        guard var components = URLComponents(string: "xtoolrunner://livecontainer-launch") else {
    ...
    private func buildLaunchURL(for item: ShareLaunchItem, launchURLString: String?) -> URL? {
        var schemeToLaunch = "xtoolrunner"
        var newLaunch = false
        ...
        if var runningLC = LCSharedUtils.getContainerUsingLCScheme(withFolderName: item.container.folderName) {
            ...
            newLaunch = true
            schemeToLaunch = item.app.isShared ? (firstFreeInstalledLC() ?? "xtoolrunner") : "xtoolrunner"
        }
